module shapes::examples::ImportGraph
...
public list[list[value]] getImportGraph() {
    loc project = |project://rascal-eclipse|;
    println(project);
    //M3 model = createM3FromEclipseProject(project);
...
public void dagre(){
	Figure b(str label) =  box(fig = text("0"), fillColor="whitesmoke", rounded=<5,5>, gap=<5,5>, grow=1.2, tooltip=label);
	list[list[value]] r = getImportGraph();
	set[str] d = {a|e<-r, str a:=e[0]}+{a|e<-r, str a:=e[1]};
	// println(d);
	states = [<e, b(e)>|e<-d];
    edges = [edge(a, z)|e<-r, str a:=e[0], str z:=e[1]];			
  	render(graph(nodes=states, edges=edges, size=<9800, 800>));
}
